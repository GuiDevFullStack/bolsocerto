...
interface EditBillModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (id: string, updates: Partial<Bill>) => void;
  bill: Bill | null;
  categories: Category[];
}

export function EditBillModal({ isOpen, onClose, onSave, bill, categories }: EditBillModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!bill || !name.trim() || !amount || !category) return;
          ...
          >
            ...
            <div className="flex items-center justify-between p-4 sm:p-6 border-b border-border">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-lg sm:text-xl font-bold">Editar Conta</h2>
              </div>
              <Button
                variant="ghost"
                size="icon"
            ...
            <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4">
              ...
              {/* Due Day (for fixed costs) or Due Date (for one-time bills) */}
              {bill.isFixed ? (
                <div className="space-y-2">
